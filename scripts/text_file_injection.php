...
// ---- LOG FUNCTION ----
$logDir = '/var/www/html/phone_extractor/logs/';
if (!is_dir($logDir)) {
    mkdir($logDir, 0777, true); // create logs directory if not exists
}
$logFile = $logDir . 'batch_import_' . date('Y-m-d') . '.log';
...
if ($dh = opendir($directory)) {
    while (($file = readdir($dh)) !== false) {
        ...
        writeLog("Processing directory: $subdirectory, DB = $name_db");
        ...
        foreach ($txtFiles as $file_txt) {
            ...
            // ---- 1️⃣ UPDATE tb_batch_management ----
            ...
            writeLog("Updated tb_batch_management (batch: $batch_name, table: $table_name, phones: $phone_nombre)");

            // ---- 2️⃣ Insert file contents into target table if empty ----
            $depCode = ucfirst($department); // e.g., "Dep01"
            try {
                $pdoTarget = getDbConnection($depCode);
            } catch (Exception $e) {
                echo "❌ Could not connect to target DB: " . $e->getMessage() . "\n";
                writeLog("Failed to connect to DB $depCode: " . $e->getMessage());
                continue;
            }

            // Check if table is empty
            $rowCountSQL = "SELECT COUNT(*) FROM `$table_name`";
            $stmtCount = $pdoTarget->query($rowCountSQL);
            $rowsInTable = (int)$stmtCount->fetchColumn();
            ...
            if ($rowsInTable === 0) {
                ...
                writeLog("Inserted $phone_nombre contacts into table `$table_name`");
            } else {
                echo "⚠️ Table `$table_name` already contains $rowsInTable rows. Skipping insert.\n";
                writeLog("Skipped insert: table `$table_name` already contains $rowsInTable rows");
            }
        }
    }
    closedir($dh);
